...
@section('title', 'Lịch sử thuê Player')
...
<div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
    ...
    <div class="bg-white dark:bg-[#2d2d2d] rounded-xl shadow-md overflow-hidden">
                    ...
                    <tr class="border-b border-slate-100 dark:border-slate-700/50 hover:bg-slate-50 dark:hover:bg-slate-800/40">
                        <td class="px-6 py-4 font-medium text-slate-900 dark:text-white whitespace-nowrap">
                            <div class="flex items-center gap-3">
                                <img src="/project/{{$detail['bplayer_image']}}" alt="Player" class="w-10 h-10 rounded-full object-cover">
                                <div>
                                    <div class="font-bold">{{$detail['bplayer_name']}}</div>
                                    <div class="text-xs text-slate-500">Đặt lúc: {{$detail['created_at']}}</div>
                                </div>
                            </div>
                        </td>
                        <td class="px-6 py-4">
                        ...
                        <td class="px-6 py-4 font-semibold">
                            {{$detail['amount']}} <i class="fas fa-gem text-[#f48024]"></i>
                        </td>
                        <td class="px-6 py-4">
                        ...
                        <td class="px-6 py-4 text-right">
                            @if($detail['status'] == 'accepted')
                                <form action="/project/rent-confirm/{{ $detail['id'] }}" method="POST">
                            ...
                            @else
                                <a href="/project/bplayer/{{$detail['bplayer_id']}}" class="action-button-details">Xem chi tiết</a>
                            @endif
                        </td>
                    </tr>
                    @empty
        ...
        <div class="p-4 border-t border-slate-100 dark:border-slate-700/50">
            @include('components.pagination', ['totalPages' => $totalPages, 'currentPage' => $currentPage])
        </div>
        @endif
    </div>
</div>
